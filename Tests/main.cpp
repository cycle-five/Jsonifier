...
template<uint64_t maxIterationCount, uint64_t minIterationCount, typename function_type> inline benchmark_results collectMape(function_type&& lambda) {
	...
	if (medianAbsolutePercentageError >= 0.05) {
		...
		return collectMape<maxIterationCount * 2, minIterationCount>(newFunction);
	} else {
		return { durations[currentIterationCount - 1], medianAbsolutePercentageError };
	}
}
...
struct json_test_helper<json_library::jsonifier, test_type::parse_and_serialize, test_data_type, minified, iterations> {
	static auto run(const std::string& newBuffer, const std::string& testName, bool doWePrint = true) {
		...
		auto writeResult = benchmark<iterations>([&]() {
			...
			newBufferSize = newerBuffer.size();
			doNotOptimizeAway(newBufferSize);
		});
...
struct json_test_helper<json_library::glaze, test_type::parse_and_serialize, test_data_type, minified, iterations> {
	static auto run(const std::string& newBuffer, const std::string& testName, bool doWePrint = true) {
		...
		auto readSize	 = buffer.size();
		auto writeResult = benchmark<iterations>([&]() {
			auto ec		  = glz::write<glz::opts{ .skip_null_members = false, .prettify = !minified, .minified = minified, .force_conformance = true }>(testData, newerBuffer);
			newBufferSize = newerBuffer.size();
			doNotOptimizeAway(newBufferSize);
		});
...
struct json_test_helper<json_library::simdjson, test_type::parse_and_serialize, test_data_type, minified, iterations> {
	static auto run(const std::string& newBuffer, const std::string& testName, bool doWePrint = true) {
		...
		buffer			= parserNew.serializeJson<jsonifier::serialize_options{ .prettify = !minified }>(testData);
...
static const std::string section001{ R"(
 > At least )" +
	...
	R"( iterations on a 6 core (Intel i7 9750h), until Median Absolute Percentage Error (Mape) reduced below 5.0%.
)" };
...
int32_t main() {
	try {
		...
		jsonifier::jsonifier_core parser{};
		file_loader fileLoader00{ basePath + "/Results.json" };
		...
		file_loader fileLoader04{ basePath + "/DiscordData-Prettified.json" };
		std::string discordData{ fileLoader04.operator std::string() };
		file_loader fileLoader05{ basePath + "/DiscordData-Minified.json" };
		std::string discordMinifiedData{ fileLoader05.operator std::string() };
		file_loader fileLoader06{ basePath + "/CanadaData-Prettified.json" };
		std::string canadaData{ fileLoader06.operator std::string() };
		file_loader fileLoader07{ basePath + "/CanadaData-Minified.json" };
		std::string canadaMinifiedData{ fileLoader07.operator std::string() };
		file_loader fileLoader08{ basePath + "/TwitterData-Prettified.json" };
		std::string twitterData{ fileLoader08.operator std::string() };
		file_loader fileLoader09{ basePath + "/TwitterData-Minified.json" };
		std::string twitterMinifiedData{ fileLoader09.operator std::string() };
		...
		fileLoader00.saveFile(resultsStringJson);
